...
BRP.bosses = {
    ...
    [2] = {
        name = GetString(AL_BRP_FIRST),
    },
    [3] = {
        name = GetString(AL_BRP_SECOND),
    },
    [4] = {
        name = GetString(AL_BRP_THIRD),
    },
    [5] = {
        name = GetString(AL_BRP_FOURTH),
    },
    [6] = {
        name = GetString(AL_BRP_FIFTH),
    },
}
...
function BRP.Init()
    ...
    EVENT_MANAGER:RegisterForUpdate(AL.name .. BRP.tag .. "MovementLoop", 2000, BRP.OnMovement)
    EVENT_MANAGER:RegisterForEvent(AL.name .. BRP.tag, EVENT_PLAYER_COMBAT_STATE, BRP.OnCombatChange)

    EVENT_MANAGER:RegisterForEvent(AL.name .. BRP.tag .. "PortalSpawn", EVENT_COMBAT_EVENT, BRP.OnPortalSpawn)
    EVENT_MANAGER:AddFilterForEvent(AL.name .. BRP.tag .. "PortalSpawn", EVENT_COMBAT_EVENT, REGISTER_FILTER_ABILITY_ID,
        BRP.PORTALID)
    EVENT_MANAGER:RegisterForEvent(AL.name .. BRP.tag .. "LastWave", EVENT_DISPLAY_ANNOUNCEMENT, BRP.OnLastWave)
end

function BRP.Reset()
    EVENT_MANAGER:UnregisterForEvent(AL.name .. BRP.tag, EVENT_PLAYER_COMBAT_STATE)
    EVENT_MANAGER:UnregisterForUpdate(AL.name .. BRP.tag .. "MovementLoop")
    EVENT_MANAGER:RegisterForEvent(AL.name, EVENT_BOSSES_CHANGED, AL.OnBossChange)

    EVENT_MANAGER:UnregisterForEvent(AL.name .. BRP.tag .. "PortalSpawn")
    EVENT_MANAGER:UnregisterForEvent(AL.name .. BRP.tag .. "LastWave")
end
...
-- https://ALw.esoui.com/downloads/info2181-BlackrosePrisonHelper.html
function BRP.OnLastWave(_, title)
    if title == GetString(AL_BRP_FINALROUND) then
        BRP.currentRound = 5
        BRP.currentWave = 0
    else
...
function BRP.Wave()
    ...
    -- STAGE 1
    if stage == 1 then
        if round == 4 and wave == 3 then AL.OnBossChange(_, true, GetString(AL_BRP_FIRST)) end

        -- STAGE 2
    elseif stage == 2 then
        if round == 4 and wave == 2 then AL.OnBossChange(_, true, GetString(AL_BRP_SECOND)) end

        -- STAGE 3
    elseif stage == 3 then
        if round == 4 and wave == 3 then AL.OnBossChange(_, true, GetString(AL_BRP_THIRD)) end

        -- STAGE 4
    elseif stage == 4 then
        if round == 4 and wave == 3 then AL.OnBossChange(_, true, GetString(AL_BRP_FOURTH)) end

        -- STAGE 5
    elseif stage == 5 then
        if round == 4 and wave == 3 then AL.OnBossChange(_, true, GetString(AL_BRP_FIFTH)) end
    end
end
